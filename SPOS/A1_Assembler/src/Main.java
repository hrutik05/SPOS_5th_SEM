...
class Pass1 {
    public static void main(String args[]) throws Exception {
        FileReader FP = new FileReader("C:\\Users\\hruti\\Desktop\\SPOS\\A1_Assembler\\1st.txt");
        ...
        while ((line = bufferedReader.readLine()) != null) {
            String[] tokens = line.split("\\s+"); // Splitting the line by tab

            // For the first line (START directive)
            if (line_count == 0 && tokens.length > 1) {
                LC = Integer.parseInt(tokens[1]); // set LC to operand of START
            } else if (line_count == 0 && tokens.length <= 1) {
                System.err.println("Error: START directive is missing an operand.");
                return;
            }

            // For subsequent lines
            else {
                for (int i = 0; i < tokens.length; i++) { // Print the input program
                    System.out.print(tokens[i] + "\t");
                }
                System.out.println("");

                // Inserting into the Symbol Table if the first token is not empty
                ...
                // Entry into symbol table for declarative statements (DS or DC)
                ...
                // Handling literals (checking if tokens[2] exists before accessing it)
                if (tokens.length >= 3 && tokens[2] != null && tokens[2].charAt(0) == '=') {
                    LitTab[litTabLine][0] = tokens[2];
                    LitTab[litTabLine][1] = Integer.toString(LC);
                    litTabLine++;
                }

                // Entry of Mnemonics into opcode table
                if (tokens.length >= 2 && tokens[1] != null) {
                    OpTab[opTabLine][0] = tokens[1];

                    // Checking assembler directives and categorizing them
